...
FileSpec HostInfoEmscripten::GetDefaultShell() {
  return FileSpec("/system/bin/sh");
}

FileSpec HostInfoEmscripten::ResolveLibraryPath(const std::string &module_path,
                                             const ArchSpec &arch) {
...
bool HostInfoEmscripten::ComputeTempFileBaseDirectory(FileSpec &file_spec) {
